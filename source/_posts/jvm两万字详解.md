...
Serial Old 也可以用在服务端程序，主要有两种用途：一种是与 Parallel Scavenge 回收器搭配使用，另外一种就是**作为 CMS 回收器发生失败时的后备预案**，在并发收集发生 **Concurrent Mode Failure** 时使用。
...
#### Parallel Scanvenge
Parallel Scavenge 是新生代回收器，采用复制算法实现，也是能够并行回收的多线程回收器。Parallel Scavenge 主要关**注可控制的吞吐量**，其它回收器的关注点是尽可能地缩短垃圾回收时的停顿时间。吞吐量就是处理器用于运行程序代码的时间与处理器总消耗时间的比值，总消耗时间等于运行程序代码的时间加上垃圾回收的时间。
...
#### Cms
concurrent mark sweep是一种以**获取最短回收停顿时间**为目标的收集器。
CMS 垃圾回收总体分为四个步骤：
...
最耗时的并发标记和并发清除阶段是和用户线程并发进行的，总体上来说，CMS 回收过程是与用户线程一起并发执行的，是一款**并发低停顿**的回收器。
​
